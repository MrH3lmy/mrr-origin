...
5. **Supported/unsupported gauges counted historical inventory, not current state (second review
   round).** `customer_mrr_snapshots` is itself an append-only history: `replay()` never deletes or
   updates a real prior instant, so a customer unsupported at T1 who later becomes supported at T2
   still has that T1 row forever. The initial fix for finding 2 queried `COUNT(*) WHERE
supported=false` over the whole table -- a monotonically-accumulating historical count, not "how
   many customers are unsupported right now," which would have kept
   `MrrCalculationUnsupportedSnapshotsElevated` firing indefinitely after every affected customer
   recovered. Fixed by restricting both gauges to each customer's single latest row via `SELECT
DISTINCT ON (workspace_id, stripe_customer_id) ... ORDER BY workspace_id, stripe_customer_id,
effective_at DESC`, then counting only among those latest rows (`RevenueCalculationSnapshotMetrics`,
   `LATEST_STATE_PER_CUSTOMER`). Multiple same-instant currency rows for one customer always share the
   same `supported` flag by construction, so `DISTINCT ON` picking any one of them is safe. The
   invocation-level `mrrorigin.revenue.calculation.invocations` counter (finding 2's other half) is
   unchanged and remains non-inflating. Regression test:
   `RevenueCalculationMetricsIntegrationTests#aCustomerRecoveringFromUnsupportedIsNoLongerCountedInTheCurrentStateGauge`.
...
| `mrrorigin.revenue.calculation.supported_snapshots`            | Gauge                            | (none)                                                           | `RevenueCalculationSnapshotMetrics` (live query: latest row per `(workspace_id, stripe_customer_id)`, `COUNT(*) WHERE supported=true`)                              | Count of customers currently supported, as of each customer's single most recent `effective_at` -- not a historical event count, aggregated across every workspace/customer.                                                                                            |
| `mrrorigin.revenue.calculation.unsupported_snapshots`          | Gauge                            | `reason` (`UnsupportedReason` enum, lowercased)                  | `RevenueCalculationSnapshotMetrics` (live query: latest row per `(workspace_id, stripe_customer_id)`, `COUNT(*) WHERE supported=false GROUP BY unsupported_reason`) | Count of customers currently unsupported for each reason, as of each customer's single most recent `effective_at` -- a customer's earlier, since-resolved unsupported instant is excluded, aggregated across every workspace/customer.                                  |
...
| `StripeWebhookOldestPendingEventTooOld` / `...Critical` | warning / critical | Oldest PENDING event >1h / >6h old                                                                                             | Is `StripeWebhookNormalizationScheduler` still ticking (`mrrorigin.billing.webhook-normalization.enabled`)? Check application logs for repeated tick failures. | Same as above; if the scheduler is disabled or failing, an operator falls back to `POST .../stripe-webhook-events/replay-failed` while investigating.                                                                                                                              |
...
calculation.unsupported_snapshots` counts each customer's _current_ latest-known state (review fix:
  restricted to one row per customer via `DISTINCT ON (workspace_id, stripe_customer_id) ... ORDER BY
  effective_at DESC`), not historical events or a monotonically-growing inventory -- there is no
  meaningful "rate" to threshold, and a customer who recovers drops back out of this count. 10 is a
  floor above whatever small number of customers are normally, currently unsupported for a
  private-beta workspace set.
...
- **`StripeWebhookNormalizationScheduler`** (`com.mrrorigin.billing`) calls the existing
  ...
  per tick. Configuration (`mrrorigin.billing.webhook-normalization.*`, all overridable by env var --
  see `application.yml`): `enabled` (default `true`), `batch-size` (default 50), `max-batches-per-tick`
  (default 10, so up to 500 events/tick), `fixed-delay`/`initial-delay` (default `PT30S` each).
- **`AttributionRecalculationScheduler`** (`com.mrrorigin.attribution`) discovers every (workspace,
...
mechanisms these services already used before #92, so they remain safe under multiple application
replicas and overlapping scheduler ticks exactly as `ARCHITECTURE.md`'s "scheduled work uses database
leases" rule requires. The scheduler never calls `restart` -- a `COMPLETED` scope simply drops out of
`pendingScopes`, so an automatic restart of a completed sweep is structurally impossible, not just
avoided by convention. See `docs/operations/recovery-runbook.md#automatic-background-processing-92`
for what changes about the operator recovery path (short version: **nothing** -- every endpoint below
still works exactly as documented; the scheduler is simply expected to make the same calls
automatically most of the time).

**When automatic processing fails or is disabled**: an operator sets the relevant `enabled` flag to
`false` (redeploy required -- there is no runtime toggle endpoint) to pause a misbehaving driver, or
falls back to the manual `resume`/`replay-failed` endpoints documented in the recovery runbook while
investigating -- those endpoints are unchanged and remain the ground truth recovery path if the
scheduler itself is the problem (e.g. a bug causes every tick to throw).

## Known limitations (flagged, not fixed by this slice)

_(Both items previously listed here -- "no scheduler drives Stripe webhook normalization" and "no
scheduler drives attribution recalculation `resume`" -- are resolved by #92 above.)_
