...
class RevenueCalculationIntegrationTests {
 ...
 @Test void approvedGoldenSnapshotsExecuteAgainstTheProductionEngine(){
  int index=0;for(JsonNode testCase:golden.path("snapshotCases")){String id=testCase.path("id").asText(),customer="snapshot-"+index++;OffsetDateTime at=OffsetDateTime.parse("2026-07-01T00:00:00Z");String currency=testCase.path("currency").asText(golden.path("currency").asText());List<Item> items=new ArrayList<>();if(testCase.has("items")){int itemIndex=0;for(JsonNode node:testCase.path("items"))items.add(new Item("item-"+itemIndex++,currency,node.path("periodAmount").asLong(),BigDecimal.valueOf(node.path("quantity").asLong()),node.path("interval").asText(),node.path("intervalCount").asInt(),false));}else items.add(new Item("item",currency,testCase.path("periodAmount").asLong(),BigDecimal.valueOf(testCase.path("quantity").asLong()),testCase.path("interval").asText(),testCase.path("intervalCount").asInt(),false));List<Discount> discounts=new ArrayList<>();if(testCase.has("discountPercent"))discounts.add(new Discount("discount",null,testCase.path("discountPercent").decimalValue(),null,null,at.minusDays(1),at.plusDays(1)));if(testCase.has("discountAmount"))discounts.add(new Discount("discount",null,null,testCase.path("discountAmount").asLong(),currency,at.minusDays(1),at.plusDays(1)));service.recordAndReplay(new SubscriptionState(w,customer,"sub",at,testCase.path("state").asText(),"golden-snapshot-"+id,items,discounts));assertThat(service.snapshots(w,customer)).as(id).singleElement().satisfies(result->{assertThat(result.supported()).isTrue();assertThat(result.amountMinor()).isEqualTo(testCase.path("expectedMrr").asLong());});}
 }
 @Test void everyApprovedGoldenMovementTypeExecutesAgainstTheProductionEngine(){
  int index=0;for(JsonNode testCase:golden.path("movementCases")){String id=testCase.path("id").asText(),customer="movement-"+index++;OffsetDateTime at=OffsetDateTime.parse(testCase.path("effectiveAt").asText());long before=testCase.path("before").asLong(),after=testCase.path("after").asLong();boolean ever=testCase.path("everPositiveBefore").asBoolean();List<SubscriptionState> states=new ArrayList<>();if(before>0)states.add(state(customer,"sub",at.minusDays(2).toString(),"active","golden-before-"+id,item("item","USD",before,1,"month",1)));else if(ever){states.add(state(customer,"sub",at.minusDays(4).toString(),"active","golden-ever-"+id,item("item","USD",Math.max(after,1),1,"month",1)));states.add(state(customer,"sub",at.minusDays(2).toString(),"canceled","golden-zero-"+id,item("item","USD",Math.max(after,1),1,"month",1)));}if(after>0)states.add(state(customer,"sub",at.toString(),"active","golden-after-"+id,item("item","USD",after,1,"month",1)));else states.add(state(customer,"sub",at.toString(),"canceled","golden-after-"+id,item("item","USD",Math.max(before,1),1,"month",1)));service.recordAndReplay(states);List<Movement> atTarget=service.movements(w,customer).stream().filter(m->m.effectiveAt().toInstant().equals(at.toInstant())).toList();if("NONE".equals(testCase.path("expectedType").asText()))assertThat(atTarget).as(id).isEmpty();else assertThat(atTarget).as(id).singleElement().satisfies(m->{assertThat(m.type()).isEqualTo(testCase.path("expectedType").asText());assertThat(m.amountMinor()).isEqualTo(testCase.path("expectedAmount").asLong());});}
 }
