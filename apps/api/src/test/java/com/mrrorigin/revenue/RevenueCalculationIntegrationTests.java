...
class RevenueCalculationIntegrationTests {
 ...
 @Test void itemScopedPercentageDiscountDoesNotDiscountSiblingItems(){
  var at=OffsetDateTime.parse("2026-01-01T00:00:00Z");var discount=new Discount("discount","discounted",BigDecimal.valueOf(50),null,null,at.minusDays(1),at.plusDays(1),false);service.recordAndReplay(new SubscriptionState(w,"cus","sub",at,"active","scoped-discount",List.of(item("discounted","USD",1000,1,"month",1),item("full-price","USD",1000,1,"month",1)),List.of(discount)));assertThat(service.snapshots(w,"cus")).singleElement().extracting(Snapshot::amountMinor).isEqualTo(1500L);
 }
