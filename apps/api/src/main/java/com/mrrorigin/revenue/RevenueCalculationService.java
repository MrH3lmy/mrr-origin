...
public class RevenueCalculationService {
 ...
 // P6 observability slice (#28): result/reason are small bounded enums (calculation outcome / UnsupportedReason),
 // never a workspace/customer id -- see docs/operations/observability-runbook.md. "unsupported" is deliberately
 // counted separately from "failure": an explicit UnsupportedReason is not an error, it's the documented outcome
 // for input this engine intentionally does not support (preserved as-is, never silently converted to success).
 private static final String RESULT_METRIC="mrrorigin.revenue.calculation.result";
 private static final String UNSUPPORTED_REASON_METRIC="mrrorigin.revenue.calculation.unsupported_reason";
 private static final String FAILURE_METRIC="mrrorigin.revenue.calculation.failure";
 ...
 // practice.
 private final Counter supportedCounter;
 private final Counter unsupportedCounter;
 private final Counter failureCounter;
 private final Timer durationTimer;
 private final Map<UnsupportedReason,Counter> unsupportedReasonCounters=new EnumMap<>(UnsupportedReason.class);
 public RevenueCalculationService(JdbcClient db,MeterRegistry meterRegistry){
  this.db=db;
  this.meterRegistry=meterRegistry;
  this.supportedCounter=Counter.builder(RESULT_METRIC).tag("result","supported").register(meterRegistry);
  this.unsupportedCounter=Counter.builder(RESULT_METRIC).tag("result","unsupported").register(meterRegistry);
  this.failureCounter=Counter.builder(FAILURE_METRIC).register(meterRegistry);
  this.durationTimer=Timer.builder(DURATION_METRIC).register(meterRegistry);
  for(var reason:UnsupportedReason.values())unsupportedReasonCounters.put(reason,Counter.builder(UNSUPPORTED_REASON_METRIC).tag("reason",reason.name().toLowerCase()).register(meterRegistry));
 }
 ...
 @Transactional public void recordAndReplay(List<SubscriptionState> states){
  ...
  try{recordAndReplayTimed(states);}
  catch(RuntimeException failure){failureCounter.increment();throw failure;}
