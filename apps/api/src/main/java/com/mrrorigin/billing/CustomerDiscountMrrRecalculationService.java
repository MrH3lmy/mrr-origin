 ...
 *
 * <p><b>Known limitation</b> (documented, not silently papered over -- matching ADR-0010's own
 * practice): a customer-level discount created before a customer's subscription starts is not
 * retroactively applied by that subscription's own {@code customer.subscription.created}
 * recalculation, which only knows its own embedded {@code discounts} array (PR #85). Closing that
 * gap would mean reopening the subscription-triggered path this issue is explicitly scoped away
 * from.
 */
@Service
