...
export function AppShell({ initialWorkspaces, children }: AppShellProps) {
  ...
  const retentionHref =
    currentWorkspaceId && currentProjectId
      ? `/app/${currentWorkspaceId}/projects/${currentProjectId}/retention`
      : undefined;
  const isRetentionActive =
    Boolean(retentionHref) && pathname === retentionHref;

  const customersHref =
    currentWorkspaceId && currentProjectId
      ? `/app/${currentWorkspaceId}/projects/${currentProjectId}/customers`
      : undefined;
        ...
        <ul className={styles.navGroup}>
          ...
          <li>
            {retentionHref ? (
              <Link
                href={retentionHref}
                className={`${styles.navLink} ${isRetentionActive ? styles.navLinkActive : ""}`}
                aria-current={isRetentionActive ? "page" : undefined}
              >
                Retention
              </Link>
            ) : (
              <span className={styles.navLinkDisabled}>Retention</span>
            )}
          </li>
          <li>
            {customersHref ? (
              <Link
                href={customersHref}
                className={`${styles.navLink} ${isCustomersActive ? styles.navLinkActive : ""}`}
                aria-current={isCustomersActive ? "page" : undefined}
              >
                Customers
              </Link>
            ) : (
